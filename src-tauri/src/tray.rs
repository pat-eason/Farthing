...
/// The bundled menu-bar glyph (bird-only template PNG). Used at setup and to
/// restore the icon after the stacked budget readout (`tray_render`) replaces
/// the status button image in budget mode.
...
pub fn setup(app: &mut tauri::App) -> tauri::Result<()> {
    ...
    // Dedicated menu bar glyph: black+alpha template image (bird-only
    // silhouette derived from the bird art, art/tray-source-bird.png; the
    // full coin is an illegible disc at menu bar sizes), so macOS recolors
    // it for dark/light menu bars (`icon_as_template` below). 82x64: macOS
    // renders the tray image 18pt tall (width proportional), so the 64px
    // height bakes in @2x retina detail. Regenerated by
    // scripts/generate-tray-candidates.py.
...
mod tests {
    ...
    /// embedded PNG (scripts/generate-tray-candidates.py output) actually
    /// decodes and is the expected template size (82x64: rendered 18pt tall
    /// by macOS with proportional width, 64px height = @2x retina detail).
    #[test]
    fn embedded_tray_template_icon_decodes() {
        let icon = tauri::image::Image::from_bytes(include_bytes!("../icons/tray-icon.png"))
            .expect("tray-icon.png decodes");
        assert_eq!((icon.width(), icon.height()), (82, 64));
    }
