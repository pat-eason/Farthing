...
pub fn setup(app: &mut tauri::App) -> tauri::Result<()> {
    ...
    // silhouette derived from the Farthing coin art; the full coin is an
    // illegible disc at menu bar sizes), so macOS recolors it for dark/light
    // menu bars (`icon_as_template` below). 80x64: macOS renders the tray
    // image 18pt tall (width proportional), so the 64px height bakes in @2x
    // retina detail. Regenerated by scripts/generate-icons.py.
...
mod tests {
    ...
    /// is the expected template size (80x64: rendered 18pt tall by macOS
    /// with proportional width, 64px height = @2x retina detail).
    #[test]
    fn embedded_tray_template_icon_decodes() {
        let icon = tauri::image::Image::from_bytes(include_bytes!("../icons/tray-icon.png"))
            .expect("tray-icon.png decodes");
        assert_eq!((icon.width(), icon.height()), (80, 64));
    }
